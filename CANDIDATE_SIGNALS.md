...
| 2 | ~~**Anchor / pre-IPO lock-in expiry overhang**~~ **TESTED → CONDITIONAL (2026-09-24).** Real, control-verified dip T-1→T+2 (90d −1.25%, t=−4.6; 30d −0.7%; placebo ~0; holds 2022-26). Not shortable by retail (new IPOs not in F&O) → avoid/exit-timing lens `lockin_expiry`. Pre-IPO 6-month lock-in **tested 2026-09-26 → NULL** (`ipo_unlock`, §19: no unlock dip, −0.35% n.s.; entering after it loses −13% median over 12 months, same as month 3 / 9). 18-month promoter lock-in not tested. See `CONCLUSIONS.md` §6. | 🟢 | ✅ done | ~~plausible edge~~ → **real, unshortable** | — |
...
| 11 | ~~**IPO listing-day & post-listing drift**~~ **TESTED → THIN / watch (2026-09-26, `ipo_listing`).** Reframed: the retail quota IS capturable — as a lottery. Mainboard n=416: one application = P(allot) x gain at the open = +1.6% (~₹237) pooled, ~₹34 in 2025-26; ≤2x retail loses; post-listing drift null (mainboard) / fat-tailed (SME, median −14% at 1y). Pre-listing GMP predicts the open (ρ 0.87). See `CONCLUSIONS.md` §18. | 🟡 | ✅ done | ~~listing pop uncapturable~~ → **small positive lottery, decayed** | — |
...
| 25 | **GMP at application time** (logged 2026-09-26) — does the grey-market premium seen by the issue close (when you must apply) pick the IPOs worth applying to, per application after allotment odds? Untestable historically: investorgain keeps only the last ~3 post-close quotes before 2026 (n=25 mainboard with a pre-close quote: ρ 0.79 with the listing gain, GMP ≥ 20% → 0 of 11 below issue). `refresh_gmp.py` now captures the full series daily. | 🟡 | ⏳ forward capture since 2026-09-26 | GMP separates losers, but hot GMP = low odds, so EV per application may barely move | Re-run `validate_ipo.py` Q2 after ~6-12 months (≥ 150 mainboard issues with a decision GMP). |
