...
def test_daily_steps_cover_events_and_self_heal_deals():
    ...
    assert s.index(["refresh_gmp.py"]) > s.index(["refresh_events.py", "ipos"])  # GMP for the IPOs just seen
    ...
    # the consolidation scan reads today's bhavcopy month, written by the price refresh
    assert s.index(["-m", "scanner.run", "consolidation", "--save"]) > s.index(["refresh_prices.py"])
