"""Refresh the NSE filings index (F1). Idempotent upserts on seq_id.
    ...
    python scripts/refresh_filings.py --from 2020-01-01 --to 2023-12-31 --ratings-only
"""
...
from scanner.filings import fetch_range, is_rating, keep, parse_announcements  # noqa: E402
...
def main():
    ...
    ap.add_argument("--ratings-only", action="store_true", help="only credit-rating filings (history backfill)")
    a = ap.parse_args()
    ...
    for f, t, raw in fetch_range(frm, a.to):
        ...
        if a.ratings_only:
            rows = [r for r in rows if is_rating(r["category"])]
